...
# Environment handling for both local and Streamlit Cloud
def get_api_keys():
    """Get API keys from Streamlit secrets (cloud) or .env file (local)."""
    openai_key = None
    anthropic_key = None
    ...
    try:
        ...
        # Fallback to environment variables (for local/Codespaces)
...
class AIProvider:
    ...
    def __init__(self):
        ...
        self.openai_key, self.claude_key = get_api_keys()
    ...
    def _claude_generate(self, prompt: str, model: str, max_tokens: int, temperature: float) -> str:
        """Claude content generation with updated model names and error handling"""
        try:
            # Updated model mapping with current Claude model names
            model_map = {
            ...
            model_id = model_map.get(model, "claude-3-5-sonnet-20241022")
            
            # Make sure anthropic_client is initialized
            ...
            st.error(f"Attempted model: {model_id}")
            
            # If the model name is wrong, try alternative names
            if "not_found_error" in str(e) or "404" in str(e):
                st.error("Model not found. Trying alternative model names...")
                try:
                    # Try with latest model names
                    alternative_models = {
                        "Claude 3.5 Sonnet": "claude-3-5-sonnet-latest",
                        "Claude 3 Haiku": "claude-3-haiku-latest"
                    }
                    alt_model = alternative_models.get(model, "claude-3-5-sonnet-latest")
                    ...
                    st.success(f"Success with alternative model: {alt_model}")
                    return response.content[0].text
                    
                except Exception as e2:
                    st.error(f"Alternative model also failed: {str(e2)}")
                    st.info("💡 Try using OpenAI models while we debug Claude integration")
...
def main():
    ...
    # Initialize API keys early
    openai_key, anthropic_key = get_api_keys()
    
    if not openai_key or not anthropic_key:
        st.error("🔑 **API Keys Missing!**")
        st.error("For local development: Add keys to .env file")
        st.error("For Streamlit Cloud: Add keys in Advanced Settings → Secrets")
        st.stop()
    ...
    # Comment out video pipeline if not available
    ...
    # Platform Optimizer (placeholder)
    elif page == "📱 Platform Optimizer":
        st.header("Platform Content Optimizer")
        st.info("This feature optimizes content for specific platform requirements and best practices.")
        
        platform = st.selectbox("Select Platform", [
            "YouTube", "TikTok", "Instagram", "Facebook", "Twitter", "LinkedIn"
        ])
        
        content_input = st.text_area("Enter your content to optimize", height=200)
        
        if st.button("🎯 Optimize Content") and content_input:
            with st.spinner(f"Optimizing for {platform}..."):
                ...
                Return optimized content that performs well on {platform}.
                """
                
                optimized = st.session_state.ai_provider.generate_content(
                    optimize_prompt, selected_model, max_tokens=1000, temperature=0.3
                )
                
                if optimized:
                    st.success(f"✅ Content optimized for {platform}!")
                    st.text_area("Optimized Content", optimized, height=300)
                    
                    st.download_button(
                        label="📥 Download Optimized Content",
                        data=optimized,
                        file_name=f"optimized_{platform.lower()}_{datetime.now().strftime('%Y%m%d')}.txt",
                        mime="text/plain"
                    )
    
    # Schedule Manager (placeholder)
    elif page == "📅 Schedule Manager":
        st.header("Content Schedule Manager")
        st.info("Schedule and manage your content calendar across platforms.")
        ...
        with col1:
            ...
            scheduled_content = [
                {"date": "2024-09-19", "platform": "YouTube", "title": "Top 5 AI Tools for Beginners", "status": "Scheduled"},
                {"date": "2024-09-20", "platform": "TikTok", "title": "ChatGPT vs Claude Quick Comparison", "status": "Draft"},
                {"date": "2024-09-21", "platform": "Instagram", "title": "AI Tool Discovery Post", "status": "Ready"},
            ]
            
            for item in scheduled_content:
                with st.expander(f"{item['date']} - {item['platform']}"):
                    st.write(f"**Title:** {item['title']}")
                    st.write(f"**Status:** {item['status']}")
                    
                    col_a, col_b, col_c = st.columns(3)
                    with col_a:
                        st.button("✏️ Edit", key=f"edit_{item['date']}")
                    with col_b:
                        st.button("📤 Post Now", key=f"post_{item['date']}")
                    with col_c:
                        st.button("🗑️ Delete", key=f"delete_{item['date']}")
        
        with col2:
            st.subheader("📊 Schedule Analytics")
            
            # Sample metrics
            st.metric("Posts This Week", "12", "2")
            st.metric("Platforms Active", "5", "0")
            st.metric("Engagement Rate", "8.5%", "1.2%")
            
            # Calendar view placeholder
            st.markdown("### 📅 Calendar View")
            st.info("Calendar integration coming soon - connect your Google Calendar or other scheduling tools.")
